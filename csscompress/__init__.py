##
# csscompress — community fork of csscompressor (sprymix/csscompressor),
# with the data-URI whitespace fix (SVG/MathML in url(...) preserved).
#
...
def _is_text_data_uri(token: str) -> bool:
    """Return True for data: URIs whose whitespace is significant.

    base64 payloads tolerate (and should have) whitespace stripped; text
    payloads (SVG, MathML, text/*) break when their whitespace is removed.
    """
    low = token.strip().strip("'\"").lower()
    if not low.startswith('data:'):
        return False
    return ';base64' not in low
...
def _preserve_call_tokens(css, regexp, preserved_tokens, remove_ws=False):
    ...
    for match in regexp.finditer(css):
        ...
        if remove_ws:
            # Preserve whitespace in text-based data URIs (SVG, MathML):
            # unlike base64, their whitespace is significant.
            if not (name.lower() == 'url' and _is_text_data_uri(token)):
